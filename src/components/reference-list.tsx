      ...
      {Object.entries(grouped).map(([category, items]) => (
        <div key={category}>
          <div className="mb-2.5 flex items-center gap-2 sm:mb-4">
            ...
            <h3 className="text-[15px] font-extrabold text-[#1a1a1a] sm:text-[18px]">
              {category}
            </h3>
          </div>
          <div className="overflow-hidden rounded-xl border border-[#eee] sm:rounded-[20px]">
            {items.map((item, idx) => (
              <a
                key={item.id}
                href={item.link}
                ...
                className={`touch-press group flex min-h-[48px] items-center justify-between gap-3 px-4 py-3 transition-colors hover:bg-[#fafafa] sm:px-6 sm:py-4 ${
                  idx < items.length - 1 ? "border-b border-[#eee]" : ""
                }`}
              >
                <span className="text-[14px] font-medium leading-snug text-[#333] group-hover:text-[#1a1a1a] sm:text-[15px]">
                  {item.title}
                </span>
